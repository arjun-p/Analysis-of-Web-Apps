	...
	for line in log:
		...
		if param[1].endswith(".jsp"):
			...
			if param[0]=='GET':
				...
				print "wget %s %s%s%s -O old%s" %(loadsession2,url_prefix,param[1].replace('empldir', 'empldirWithErrors'),param[2], jspfile)
				...
				if len(param[4])>2:
					...
					if param[1].endswith("Login.jsp"):
						...
						print "wget %s %s %s %s%s -O old%s" %(savesession2,postsyntax,post,url_prefix,param[1].replace('empldir', 'empldirWithErrors'), jspfile)
					else:
						print "wget %s %s %s %s%s -O new%s" %(loadsession1,postsyntax,post,url_prefix,param[1], jspfile)
						print "wget %s %s %s %s%s -O old%s" %(loadsession2,postsyntax,post,url_prefix,param[1].replace('empldir', 'empldirWithErrors'), jspfile)
